...
from molNet.dataloader.molecule_loader import (
    MoleculeFromCsvLoader,
    MoleculeGraphFromCsvLoader,
...
class DataLoaderTest(unittest.TestCase):
    ...
    def test_local_csv_smiles_loader_multifile__A_force_new(self):
        s = time.time()
        loader = SmilesFromCsvLoader(
            file="../datasets/delaney-processed.csv",
        ...
        for batch in loader.train_dataloader():
            ...
            assert (
                len(
                    set(batch["data"]["Compound ID"]).symmetric_difference(
                        {"2-butenal", "2-Ethyltoluene", "Triclosan"}
                    )
                )
                == 0
            )
    ...
    def test_local_csv_smiles_loader_multifile__B__reload(self):
        ...
        loader.setup()
        assert (
            DataLoaderTest.new_load_time / 10 > time.time() - s
        ), "loading took too long ({})".format(time.time() - s)
        for batch in loader.train_dataloader():
            testdata = ["CC=CC=O", "CCc1ccccc1C", "Oc1cc(Cl)ccc1Oc2ccc(Cl)cc2Cl"]
            assert batch["smiles"] == testdata
            assert (
    ...
    def test_local_csv_smiles_loader_multifile__C_on_demand(self):
        ...
        for batch in loader.val_dataloader():
            ...
            assert (
                len(
                    set(batch["data"]["Compound ID"]).symmetric_difference(
                        {"2,3,3',4,4',5-PCB", "Hexylbenzene ", "Cortisone"}
                    )
                )
                == 0
            ), batch["data"]["Compound ID"]
            break

    def test_local_csv_mol_loader_multifile__A_force_new(self):
        s = time.time()
        loader = MoleculeFromCsvLoader(
            file="../datasets/delaney-processed.csv",
            smiles_column="smiles",
            reload=True,
            additional_data="all",
        ...
        for batch in loader.train_dataloader():
            assert (
                len(
                    set(
                        [
                            d.as_dict()["properties"][DATATYPES.STRING]["Compound ID"]
                            for d in batch
                        ]
    ...
    def test_local_csv_molgraph_loader_multifile__A_force_new(self):
        s = time.time()
        loader = MoleculeGraphFromCsvLoader(
            file="../datasets/delaney-processed.csv",
            smiles_column="smiles",
            reload=True,
            additional_data="all",
        ...
        for batch in loader.train_dataloader():
            ...
            assert (
                batch[0]["properties"][DATATYPES.STRING]["smiles"] == "CCCCCCO"
            ), batch[0]["properties"][DATATYPES.STRING]["smiles"]
            assert (
                batch[1]["properties"][DATATYPES.FLOAT][
    ...
    def test_local_csv_molgraph_loader_multifile__C_on_demand(self):
        ...
        for batch in loader.train_dataloader():
            ...
            assert len(set(batch[2]["edges"]).symmetric_difference(test)) == 0, batch[
                2
            ]["edges"]
    ...
    def test_local_csv_ptgeom_molgraph_loader_multifile__A_force_new(self):
        s = time.time()
        loader = PytorchGeomMolGraphFromCsvLoader(
            file="../datasets/delaney-processed.csv",
            smiles_column="smiles",
            reload=True,
            seed=42,
        ...
        for batch in loader.train_dataloader():
            ...
            assert all(
                batch["string_data"][0][i] == test_data[i]
                for i in range(len(test_data))
            ), "strings for 0 dont match"
            test_data = ["", "CCOP(=O)(OCC)OCC", "Triethyl phosphate"]
            assert all(
                batch["string_data"][1][i] == test_data[i]
                for i in range(len(test_data))
            ), "strings for 1 dont match"
            ...
            assert test_data.size == batch["graph_features"].numpy().size, batch[
                "graph_features"
            ].numpy()
            assert np.allclose(
                test_data, batch["graph_features"].numpy().flatten()
            ), batch["graph_features"].numpy()

            test_data = np.array([-1.9500, 0.4300, -2.5100])
            assert np.allclose(test_data, batch["y"].numpy().flatten()), batch[
                "y"
            ].numpy()
            break
    ...
    def test_local_csv_ptgeom_molgraph_loader_multifile__C_on_demand(self):
        ...
        for batch in loader.train_dataloader():
            ...
            test_data = [
                "",
                "CN(C)C(=O)Nc1ccc(-n2nc(C(C)(C)C)oc2=O)c(Cl)c1",
                "Dimefuron",
            ]
            assert all(
                batch["string_data"][1][i] == test_data[i]
