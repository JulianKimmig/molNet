...
from blender_script_creator.geometry import get_or_create_object, create_plain_object, \
    BlenderObject, BlenderText, Sphere, Connection
...
from blender_script_creator.script import BlenderScript, blender_function, BlenderVariable

import numpy as np

from molNet.featurizer.atom_featurizer import atom_partial_charge, atom_mass, atom_hybridization_one_hot, \
    atom_is_aromatic, atom_symbol_hcnopsclbr_one_hot, atom_formal_charge, atom_total_degree_one_hot, \
    atom_degree_one_hot, atom_num_radical_electrons, atom_is_in_ring_size_3_to_20_one_hot
from molNet.featurizer.featurizer import FeaturizerList
from molNet.mol.molecules import Molecule, MolGraph
from molNet.third_party.blender.mol import mol_to_model, BlenderMol, get_default_atom_map
...
class BeamLayer(Connection):
    ...
    def __init__(self, obj, name, beam):
        super().__init__(obj, name, p1=(0, 0, 0),
                         p2=(0, 0, 10),
                         d=1)
        self.beam = beam
        self._ini()
    ...
    def _ini(self):
        ...
        self._beam_layer_mat, new_mat = get_or_create_material(name=self.name + "material", )

        self._em, n = self._beam_layer_mat.get_or_create_node(name="emission", type="ShaderNodeEmission")
        new_mat = new_mat or n
        self._rgbramp, n = self._beam_layer_mat.get_or_create_node(name="rgbramp", type="ShaderNodeValToRGB")
        new_mat = new_mat or n
        self._mapping, n = self._beam_layer_mat.get_or_create_node(name="mapping", type="ShaderNodeMapping")
        new_mat = new_mat or n
        mixer, n = self._beam_layer_mat.get_or_create_node(name="mixer1", type="ShaderNodeMixShader")
        new_mat = new_mat or n
        trans, n = self._beam_layer_mat.get_or_create_node(name="transparent", type="ShaderNodeBsdfTransparent")
        new_mat = new_mat or n
        noisetexture, n = self._beam_layer_mat.get_or_create_node(name="noisetexture", type="ShaderNodeTexNoise")
        new_mat = new_mat or n
        textcoord, n = self._beam_layer_mat.get_or_create_node(name="textcoord", type="ShaderNodeTexCoord")
        new_mat = new_mat or n
    ...
    def create_motion(self, animator):
        ...
        animator.change_node_value(self._mapping.Location, value=(0, 0, 0), interpolation="LINEAR")
        animator.change_node_value(self._mapping.Rotation, value=(0, 0, 0), interpolation="LINEAR")
...
class Beam(BlenderObject):
    dependencies = BlenderObject.dependencies + [BeamLayer, ]

    def __init__(self, obj, name, start=(0, 0, 0), end=(0, 0, 50), speed=0.25,forcelength=None):
    ...
    def add_layer(self, dia=1., color=(0, 0.7, 1, 1), trans_dens=0.52, wave_height=0.5):
        l = BeamLayer.get_or_create_object(beam=self, name="{}_layer_{}".format(self.name, len(self._layer)),
    ...
    @property
    def length(self):
        return np.linalg.norm(self.end - self.start) if self.forcelength is None else self.forcelength
    ...
    def end(self, loc):
        loc = np.array(loc)
        for lay in self._layer:
            lay._set_length(np.linalg.norm(self.start - loc) if self.forcelength is None else self.forcelength)
        self._end = loc
        self.set_rotation(*self.calc_rot())
...
class ThreePointBeam(BlenderObject):
    ...
    def __init__(self, obj, name, center, p1, p2, p3, ball_height=3, base_color=(0, 0, 1, 1),max_length=None):
        super().__init__(obj, name)
        ...
        self.center_ball = Sphere.get_or_create_object("{}_centerball".format(name), dia=1)
        self.center_ball.obj.scale = (1, 1, 1)
        self.center_ball.parent = self
        self.center_ball.location = [0, 0, ball_height]

        self.center_beam = Connection.get_or_create_object("{}_centerbeam".format(name), p1=np.array([0, 0, 0]),
                                                           p2=self.center_ball.location, d=0.1, resolution=50)

        self.center_beam.parent = self
        self.p1_beam = Connection.get_or_create_object("{}_p1_beam".format(name), p1=p1, p2=self.center_ball.location,
                                                       d=0.1, resolution=50)

        self.p1_beam.parent = self
        self.p2_beam = Connection.get_or_create_object("{}_p2_beam".format(name), p1=p2, p2=self.center_ball.location,
                                                       d=0.1, resolution=50)

        self.p2_beam.parent = self
        self.p3_beam = Connection.get_or_create_object("{}_p3_beam".format(name), p1=p3, p2=self.center_ball.location,
                                                       d=0.1, resolution=50)
        self.p3_beam.parent = self

        material, new_mat = get_or_create_material(name="beam_{}".format(base_color))
        emission, n = material.get_or_create_node(type="ShaderNodeEmission", name="Emission")
        new_mat = new_mat or n
        color_node, n = material.get_or_create_node(type="ShaderNodeRGB", name="colornode")
        new_mat = new_mat or n
        if new_mat:
        ...
        self.main_beam = Beam.get_or_create_object("{}_main_beam".format(name), start=self.center_ball.location,
                                                   end=(0, 0, 100), speed=0.25,forcelength=max_length
                                                   )
        self.main_beam.parent = self
        base_color = np.array(base_color, dtype=float)
        self.main_beam.add_layer(color=base_color)

        self.main_beam.add_layer(dia=0.4, color=(1, 1, 1, 1), trans_dens=0.48, wave_height=0.2)

        self.main_beam.add_layer(dia=1.75,
                                 color=base_color / 4 + np.array(
                                     [base_color[1], base_color[2], base_color[0], 8 * base_color[3]]) / 8,
                                 trans_dens=0.56)
    ...
    def _stop_move(self, animator, outdist):
        ...
        animator.scale_object(self.main_beam, 0, 0, 0., time=None, reverse=True)

        animator.move_object(self.center_ball, 0, 0, outdist, delta=True, time=None, reverse=True)
        animator.move_object(self.center_beam, 0, 0, outdist, delta=True, time=None, reverse=True)
        animator.move_object(self.p1_beam, 0, 0, outdist, delta=True, time=None, reverse=True)
        animator.move_object(self.p2_beam, 0, 0, outdist, delta=True, time=None, reverse=True)
        animator.move_object(self.p3_beam, 0, 0, outdist, delta=True, time=None, reverse=True)
        animator.move_object(self.main_beam, 0, 0, outdist, delta=True, time=None, reverse=True)
        ...
        animator.scale_object(self.center_ball, 0.1, 0.1, 0., time=None, reverse=True)
        animator.scale_object(self.center_beam, 0.1, 0.1, 0., time=None, reverse=True)
        animator.scale_object(self.p1_beam, 0.1, 0.1, 0., time=None, reverse=True)
        animator.scale_object(self.p2_beam, 0.1, 0.1, 0., time=None, reverse=True)
        animator.scale_object(self.p3_beam, 0.1, 0.1, 0., time=None, reverse=True)
    ...
    def animate(self, animator: BlenderAnimationTracker, t0=0.2, t1=1, t2=0.5, outdist=1000):
        cf = animator.current_frame
...
@blender_function(dependencies=[Node, Camera, connect_node_sockets, Wave, BlenderAnimationTracker,
                                delete_all_but, get_or_create_material, Subsurface, BlenderObject, base_mol_data,
                                BlenderMol, featuremap, feature_names, BlenderText, LightSource, ThreePointBeam])
def mol_main():
    # scene
    RENDER_EDIT = False
    DIST = 2
    ...
    def get_feat_mat(strength=1):
        ...
        feature_material, new = get_or_create_material(name="feat_mat_{}".format(strength))

        emission, n = feature_material.get_or_create_node(type="ShaderNodeEmission", name="Emission")
        new = new or n
        bsdf, n = feature_material.get_or_create_node("principled bsdf")
        new = new or n
        color_node, n = feature_material.get_or_create_node(type="ShaderNodeRGB", name="colornode")
        new = new or n
        mixer, n = feature_material.get_or_create_node(type="ShaderNodeMixShader", name="Mixer")
        new = new or n
    ...
    for l in range(len(layer_molecules)):
        for i in range(len(layer_molecules[l])):
            if l == 0:
                m = BlenderMol.get_or_create_object("mol_layer_{}_{}".format(l, i),
    ...
    for i in range(len(layer_molecules[0])):
        t = BlenderText.get_or_create_object("l0_feature_desc_{}".format(i), text=feature_names[i], size=3,
                                             align_y="BOTTOM")
    ...
    first_beam = ThreePointBeam.get_or_create_object("first_beam",
    ...
    for i in range(len(layer_molecules[0][0].atoms)):
        ...
        for j in range(LASER_MULTIPLIER_FIRST_LAYER):
            fll = ThreePointBeam.get_or_create_object("first_layer_laser_{}_{}".format(i,j),
    ...
    for i in range(len(layer_molecules[1][0].atoms)):
        ...
        for j in range(LASER_MULTIPLIER_SECOND_LAYER):
            fll = ThreePointBeam.get_or_create_object("second_layer_laser_{}_{}".format(i,j),
    ...
    for i in range(LAYER_SIZES[-1]):
        ...
        for j in range(FEATS_PER_OUT):
            feat_out=Sphere.get_or_create_object(name="feat_ball_{}_{}".format(i,j),dia=FEATBALLDIA)
            from_m.append(feat_out)
            feat_out.location = (ballsx[b],0,len(LAYER_SIZES)*LAYER_HEIGHT-LAYER_HEIGHT/2)
            b+=1
    ...
    for i in range(len(layer_molecules[1][0].atoms)):
        ...
        main_beam = Beam.get_or_create_object("feature_beam_{}".format(i), start=layer_molecules[-1][0].atoms[i].world_location,
                                                   end=featballs[0][0].world_location, speed=0.25,forcelength=LAYER_HEIGHT*2
                                                   )
        main_beam.atom_n = i
        c = np.array(c, dtype=float)
        main_beam.add_layer(color=c,dia=0.5)

        main_beam.add_layer(dia=0.05, color=(1, 1, 1, 1), trans_dens=0.48, wave_height=0.2)

        main_beam.add_layer(dia=0.6,
                                 color=c / 4 + np.array(
                                     [c[1], c[2], c[0], 8 * c[3]]) / 8,
                                 trans_dens=0.56)
    ...
    delete_all_but([base_mol, layer_molecules, cams, sun, first_beam,first_layer_lasers,second_layer_lasers,featballs,feat_lasers])
    ...
    first_layer_lasers_pack = BlenderObject.get_or_create_object("first_layer_lasers_pack")
    for l in range(len(first_layer_lasers)):
        first_layer_lasers[l].parent=first_layer_lasers_pack

    second_layer_lasers_pack = BlenderObject.get_or_create_object("second_layer_lasers_pack")
    ...
    for atom in base_mol.atoms:
        anim.change_node_value(atom.material.get_node("Emission").Strength, 30, time=0.25)
        anim.change_node_value(atom.material.get_node("Emission").Strength, 1, time=0.25)
    ...
    for i in range(len(layer_molecules[0])):
        ...
        if i >= max_mol_create:
            anim.move_object(base_mol, *(ordered[i][0], ordered[i][1], MOl_MOVE_HEIGHT), time=0.1)
        else:
            anim.move_objects([base_mol, cam], *(ordered[i][0], ordered[i][1], MOl_MOVE_HEIGHT), time=0.2)

        anim.move_object(layer_molecules[0][i], *(ordered[i][0], ordered[i][1], 0), time=None)
    ...
    anim.run_seconds(max(0.5, anim.frames_to_seconds(anim.get_frame('first_layer_complete_down') - anim.current_frame)))
    anim.save_frame("first_layer_zoom_out")

    D = 75
    for l in range(len(layer_molecules)):
        if l > 0:
            anim.move_object(layers_mols_list[l], z=-OUT_DIST + D, delta=True, time=None, reverse=True)
            # layer_mols.location = (0, 0, l * LAYER_HEIGHT + OUT_DIST)
    anim.run_frames(1)

    anim.rotate_object(cam, 90, 0, 0, time=1.9, reverse=True)
    anim.move_object(cam, 0, -LAYER_HEIGHT * 6, LAYER_HEIGHT / 2, time=1.9, reverse=True)

    anim.move_object(layers_mols_list[1], z=-D, delta=True, time=2)

    for l in range(len(layer_molecules)):
        if l > 1:
            anim.move_object(layers_mols_list[l], z=-D, delta=True, time=2, reverse=True)

    anim.move_object(cam, 0, -100, LAYER_HEIGHT * (len(layer_molecules) + 1), time=2, reverse=True)
    ...
    anim.go_to_frame(max(anim.get_frame('first_laser_finished'),anim.get_frame('post_beam_dissolved_rotation')))
    anim.save_frame("pre_under_mol_laser_flight")
    anim.run_seconds(1)

    cam.track(layer_molecules[1][0],influence=1,animator=anim,frames=anim.seconds_to_frames(1))
    ...
    cam.track(layer_molecules[1][0],influence=0,animator=anim,frames=anim.seconds_to_frames(1))
    anim.run_frames(anim.seconds_to_frames(1))
    anim.run_seconds(1)
    ...
    anim.go_to_frame(max(anim.get_frame("first_layer_laser_single_mol_done"),anim.get_frame("post_under_mol_laser_flight")))
    ...
    anim.move_object(cam,cam.location[0]/2,cam.location[1]/2,LAYER_HEIGHT*2,time=1)
    ...
    for i in range(LAYER_SIZES[-1]):
        for ball in  featballs[i]:
            for l in feat_lasers:
                ...
                for layer in l._layer:
                    anim.bevel_start_end(layer,0,0,time=None,reverse=True)
                anim.move_object(l,*layer_molecules[-1][i].atoms[l.atom_n].world_location,time=None,reverse=True)

            anim.run_frames(1)
            em = ball.material.get_node("Emission")
            for j,l in enumerate(feat_lasers):
                length=  np.linalg.norm(
                    ball.world_location - layer_molecules[-1][i].atoms[l.atom_n].world_location
                )
                bv=length/l.length
                for layer in l._layer:
                ...
                anim.change_node_value(em.Strength,1+ 15*(j+1)/len(feat_lasers),time=0,reverse=True)
