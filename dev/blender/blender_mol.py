...
from molNet.third_party.blender.tools import (
    blender_function,
...
from molNet.third_party.blender.tools.animation import (
...
from molNet.third_party.blender.tools.scene import (
    clear_scene,
    add_light,
    find_object_by_name,
)
...
def ini():
    ...
    base_mol = mol_to_model(
        mol, mol_name="base_mol", dist=DIST, varname="base_mol", seed=SEED
    )
...
def move_molecule(
    mol, with_cam=False, x=0.0, y=0.0, z=0.0, time=0.001, delta=True, animator=None
):
    _mol = mol["molecule"]
    if with_cam:
    ...
    if with_cam:
        move_objects(
            [_mol, cam], x=x, y=y, z=z, delta=delta, animator=animator, time=time
        )
    else:
...
conf = _mol.GetConformer()
coordds = (
    np.array([list(conf.GetAtomPosition(i)) for i, a in enumerate(_mol.GetAtoms())])
    * (DIST + 1)
    * 1.5
)
...
featuremap = blender_lambda_var(
    "featuremap",
    "np.array",
    [data["molNet_features"] for n, data in mol.nodes(data=True)],
    dtype=float,
)
...
def ini():
    for f in range(len(atom_featurizer)):
        ...
        fd = create_text(
            atom_featurizer.describe_features()[f], "feature_desc{}".format(f), size=3
        )
...
def scene1():
    ...
    for f in range(SHOW_MOLS):
        ...
        #                            time=0.00001
        #                            )
        move_to_show = move_object(
            layer0_copyies[f], z=-OUT_DIST, delta=True, animator=anim, time=0.00001
        )
