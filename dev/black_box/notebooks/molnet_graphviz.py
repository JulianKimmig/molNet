...
def atom_input_graph(mol_graph,name,reduced=True,reduce_up=2,graph=None):
    ...
    if reduced:
        ...
        draw_atoms=[[a,a] for a in atoms[:-1]]+[["reduce","..."],[atoms[-1],atoms[-1]]]
    else:
    ...
    g.node(node_name,
          table(np.array([[[np,ns]] for np,ns in draw_atoms])),
           shape="none"
          )
    return g,{"ports":atoms,
              "draw_atoms":draw_atoms,"reduced":reduced,"node_name":node_name}
    ...
    for i in range(input_shape[1]):
        ...
        with g.subgraph(name="cluster_{}_input{}".format(name,i)) as g:
            ...
            g.node("{}_ip_feat{}".format(name,i),
                 label=table(np.array([[["ip_{}_{}".format(i,x),"feat{} ip{}".format(x,i)]] for x in range(input_shape[0])])),
                  shape="none"
                  )
        g=pg
