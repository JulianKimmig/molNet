from molNet.featurizer.featurizer import FeaturizerList, LambdaFeaturizer, OneHotFeaturizer
...
def mol_to_polyunit_featurizer(mol_featurizer):
    return LambdaFeaturizer(lamda_call=lambda connectable_group:mol_featurizer(connectable_group.mol),length=len(mol_featurizer))
...
number_of_connections=LambdaFeaturizer(lamda_call=lambda connectable_group:[len(connectable_group.connection_indices)],length=1)
