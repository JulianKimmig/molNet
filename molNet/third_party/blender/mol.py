...
from blender_script_creator.geometry import BlenderObject, create_plain_object, Sphere, \
    Connection
...
def get_default_atom_map(atom,index=-1):
    am =  {
        "H": {
            "dia": 0.5,
            "material":default_atom_material(name="default_Atom_H",color=(1,1,1,1))
        },
        "C": {
            "dia": 1,
            "material":default_atom_material(name="default_Atom_C",color=(0.1,0.1,0.1,1))
        },
        "O": {
            "dia": 0.95,
            "material":default_atom_material(name="default_Atom_O",color=(1,0,0,1))
        },
        "N": {
            "dia": 0.95,
            "material":default_atom_material(name="default_Atom_N",color=(0,0,1,1))
        },
        "Unknown": {
            "dia": 2,
            "material":default_atom_material(name="default_Atom_Unknown",color=(0.5,0.5,0.5,1))
        },
    }
...
class BlenderMol(BlenderObject):
    dependencies = BlenderObject.dependencies + [get_default_atom_map,Sphere,Connection,BlenderAtom,get_bond_material]
    def __init__(self, obj,name,atoms, coordinates, bonds, dist=0, bond_size=0.3, atom_map=get_default_atom_map,bond_material=get_bond_material):
    ...
    def _check_atoms(self,atoms,atom_map,coordinates,dist):
        for i in range(len(atoms)):
            ...
            sphere = BlenderAtom.get_or_create_object(name="{}_atom_{}".format(self.name, i), dia=atom["dia"])
...
def mol_to_model(mol: Molecule,varname,dist=0,seed=None):
    ...
    return BlenderVariable(
        varname,dict(atoms=[a.GetSymbol() for a in _mol.GetAtoms()],
