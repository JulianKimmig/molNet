...
def generate_n_random_carbon_lattice(
    ...
    for k in range(rounds):
        ...
        for i in range(min_c, max_c + 1):
            for _ in range(i):
                temp_subs.add(generate_random_carbon_lattice(i))
        temp_subs = temp_subs - substructures
        temp_subs = list(temp_subs)
        random.shuffle(temp_subs)
        temp_subs = set(temp_subs)
        substructures.update(temp_subs)
        rtries = 0
        for i in range(int(np.sqrt(n) + 1)):
            ...
            for smiles in temp_subs.copy():
                ...
                substructures.update(temp_subs)
                temp_subs.remove(smiles)
                if len(substructures) >= n:
                    break
            if len(substructures) >= n or len(temp_subs) == 0:
                break
            if len(substructures) == prel:
                rtries += 1
            if rtries > random_tries:
                break
        if len(substructures) >= n:
            break
    substructures = list(substructures)
    random.shuffle(substructures)

    substructures = sorted(substructures[:n], key=lambda s: s.count("C"))
