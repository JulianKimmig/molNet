...
def gen_ecdf(
    ...
    data = np.array(
        [(s, gen_args, gen_kwargs, featurizer_class) for s in data], dtype=object
    )
...
if __name__ == "__main__":
    ...
    from molNet.featurizer.molecule_featurizer import (
        MolWtFeaturizer,
        NumAtomsFeaturizer,
        CrippenDescriptorsFeaturizer,
    )

    smiles = np.array(
        [k for k in generate_n_random_hetero_carbon_lattice(n=100_000, max_c=10)],
        dtype=object,
    )
