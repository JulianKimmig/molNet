...
login_page_link=soup.find("iframe", src=lambda value: value and value.startswith(BASEURL+"login/show.do;jsessionid"))['src']

soup,session = request_soup(login_page_link,session=session)


anonym_login_link = BASEURL+soup.find("a", href=lambda value: value and value.startswith("login/login.do?anonymous=1"))['href']


soup,session = request_soup(anonym_login_link,session=session)


accept_licence_link = BASEURL+soup.find("a", href=lambda value: value and value.startswith("login/acceptLicenseAgreement.do?") and value.endswith("accepted=1"))['href']
...
def get_alert_page(p,s):
    return s.post(BASEURL+"alerts/list.do",data={"out":"json",
    "pagenum":str(p),
    "render-mode":"popup",
}
    ).json()
