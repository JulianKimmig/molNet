...
testmols = [assert_conformers(m) for m in testmols]
...
print(
    timeit.timeit(
        lambda: [atom.GetAtomicNum() for atom in testmols[0].GetAtoms()], number=1000
    )
)
print(
    timeit.timeit(
        lambda: [atom.GetSymbol() for atom in testmols[0].GetAtoms()], number=1000
    )
)
...
for mod_name, func_name in sucsess_mods:
    ...
    already_in = False
    for ss in succs:
        if s["func_name"] == ss["func_name"] and s["module"] == ss["module"]:
            already_in = True
            break
    if already_in:
        continue
...
for s in succs[::-1]:
    ...
    if len(data) != len(testmols):
        data = [np.array(try_f(m)) for m in testmols]
...
print(len(testmols[0].GetAtomsMatchingQuery(rdqueries.AtomNumEqualsQueryAtom(6))))
print(
    timeit.timeit(
        lambda: len(
            testmols[0].GetAtomsMatchingQuery(rdqueries.AtomNumEqualsQueryAtom(6))
        ),
        number=10000,
    )
)
...
for s, d in sheets.items():
    ...
    for ss in d:
        red_name = ss["red_name"]
        sn = "molecule_" + red_name
        available_featurizer.append(sn)
        full_code += "{}={}()\n".format(sn, red_name + "_Featurizer")

    full_code += "\n"
    full_code += "_available_featurizer=[\n{}\n]\n".format(
        ",\n".join(["{}".format(af) for af in available_featurizer])
    )
